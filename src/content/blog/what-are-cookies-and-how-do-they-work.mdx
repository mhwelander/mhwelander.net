...
You can see the responses in [Chrome Developer Tools](https://developer.chrome.com/docs/devtools). Click `Option + ⌘ + I` (Mac) or `F12` (Windows), then click on the **Network** tab and refresh your page. Notice that the response from `mhwelander.net` includes various requests to get fonts, stylesheets, and script files:
...
In this particular example, all cookies are **first-party** - they come from Wikipedia itself. However, if you reference an external script or image, the response from that resource can include a `Set-Cookie` header that sets a **third-party cookie**:
...
For example, [TikTok](https://ads.tiktok.com/help/article/tiktok-pixel) and [Snapchat](https://businesshelp.snapchat.com/s/topic/0TO0y000000YVdJGAW/snap-pixel?language=en_US) set third-party cookies **when you load their tracking script**. This cookie is then accessed by that tracking script - on your website, and on any website you visit that has the tracking script.

<figure class= "centerFig">
<div   data-lightbox="true">
![alt text](./images/2024/snapchat-cookies.png)
</div>
<figCaption>Cookies from Snapchat</figCaption>
</figure>
...
This may seem counterintuitive - Google Analytics is a third-party script, so why isn't it creating a third-party cookie? Let's dig into what a third-party cookie actually is, and why it matters.
...
The thing that makes them third-party cookies is this: <span class="highlight">**the cookie domain does not match the current site.**</span> That's it.
...
### Who is setting these cookies?

A third-party cookie is set by a `Set-Cookie` header in a response from some <span class="highlight">**embedded resource from another domain**</span>. For example, the `_ttp` cookie (TikTok) is set by loading the `events.js` script from analytics.tiktok.com (the cookie is _not_ set by the Javascript itself).

You see this by right-clicking on a cookie and choosing **Show Requests With This Cookie** - the request to `events.js` sets the cookie through the `Set-Cookie` header:
...
Imagine that I arrived on a website by clicking an ad on TikTok. The script on the website sets a cookie, and probably associates that cookie with my TikTok handle.

The tracking script can send events to TikTok **about my behavior on that website** after seeing the ad - what did I look at? Did I buy something? Did I register? All of this behavioral data is linked to my cookie, and it:

* Helps the website track the effectiveness of their ad campaigns
* Provides TikTok with behavioral data that can help them target ads more effectively

[Image - tiktok to website to cookie]

More specifically, it works like this: the other requests that use the `_ttp` cookie (`pixel` and `act`) are initiated by a script file named `main.*.js`. They send the cookie _to_ analytics.tiktok.com:

<figure class= "centerFig">
<div   data-lightbox="true">
![alt text](./images/2024/ping-use.png)
</div>
<figCaption>Third party cookies on ███████.com</figCaption>
</figure>

These requests include the `_ttp` cookie in the `Cookies` request header:

<figure class= "centerFig">
<div   data-lightbox="true">
![alt text](./images/2024/using-the-cookie.png)
</div>
<figCaption>Third party cookies on ███████.com</figCaption>
</figure>

The request _also_ includes information about my activity in the request payload:

<figure class= "centerFig">
<div   data-lightbox="true">
![alt text](./images/2024/payload.png)
</div>
<figCaption>Third party cookies on ███████.com</figCaption>
</figure>

If you're thinking: <span class="highlight">this could all be done with a **first-party cookie**</span>, well, you're right. It absolutely could be.

### What exactly is the role of _third-party_ cookies in this?

You bring your TikTok cookie with you to <span class="highlight">**any website** that includes the TikTok tracking script</span>. When another website sends a request to analytics.tiktok.com, your browser **automatically** includes your `_ttp` cookie in the `Cookies` request header.

[Image - website / cookie / TikTok ID]

TikTok now receives behavioral data about **the same person** across two **different websites**. Or fifty websites.

This is _great_ for TikTok - for the lifetime of that cookie, they can gather data about your behavior across the internet and use that target ads more effectively.

<Block class="note">
Using TikTok as an example - all ad providers do something like this.
</Block>

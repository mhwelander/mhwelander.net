...
### Who decides which cookies a website should have?

The website owner decides which cookies a website should give to users.  Cookies can created by:

* The website itself (through code)
* An embedded script (for example, Google Analytics)
* An embedded an iframe (for example, a YouTube video)

For example, by choosing to add Google Analytics to my website (you can opt out at any time in the footer), I added a script that issues `_ga` and `_ga_*` cookies to visitors.

### Are cookies essential?

Some cookies are in fact **essential**, which means that website does not function correctly without them. For example, cookies are required to:
...
Very useful to a business, and often make your experience more tailored to you, but you do not have to accept them.

### Is the cookie there forever?

No. A **persistent cookie** lasts until you delete it, or until it expires. The website that issues the cookie sets the expiry date, but:
